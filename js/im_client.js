/* global RL_YTX, IM, $, emoji, hex_md5 */
'use strict';
/**
...
(function() {
    window.IM = window.IM || {
        ...
        EV_keyCode: function(event) {
            ...
            } else if (IM._keyCode_1 === 16 && IM._keyCode_2 === 229) {// chrome中文输入法时返回229
                setTimeout(function() {
            ...
            } else if (IM._keyCode_2 === 8) {// 退格键
                $('#groupMemList_div').hide();
            } else if (IM._keyCode_2 === 16) {// 火狐中文输入模式
        ...
        EV_resendMsg: function(obj) {
            ...
            if (msgtype === 1) {// 文本消息
                ...
                if (IM._contact_type_m === contact_type) {
                    IM.EV_sendMcmMsg(oldMsgid, text, receiver, true);
                } else {
                    IM.EV_sendTextMsg(oldMsgid, text, receiver, true);
                }

            } else if (msgtype === 4 || msgtype === 7) {
                oFile = msg.find('input[imtype="msg_attach_resend"]')[0];
                if (!!oFile) {
                    oFile = oFile.files[0];
       ...
       /* EV_syncMsg: function(sv, ev) {
        ...
        EV_getRecentConList: function() {
            ...
            RL_YTX.getRecentContactList(recentConListBuilder, function(obj) {
        ...
        DO_pop_show_help_GroupDetail: function(obj, groupId, target) {
            ...
            if (isowner || target === 1) {
                ...
                str = '<textarea class="pull-right" rows="3" style="width:95%;">'
            ...
            if (obj.mcmEvent === 0) {// 0普通im消息
                ...
                if (msgType === 1) {
                    ...
                    */
                    ...
                    //                     if (isFireMsg) {
                    //                         str = '<div style="display:inline"><a fireMsg="yes" href="' + blobURL + '" download="' + fileName + '" target="_blank">'
                    //                             + '<span>'
                    ...
                    //                     } else {
                    //                         str = '<div style="display:inline"><a href="' + blobURL + '" download="' + fileName + '" target="_blank">'
                    //                             + '<span>'
                    //                             + '<img style="width:32px; height:32px; margin-right:5px; margin-left:5px;" src="assets/img/attachment_icon.png" />'
                    //                             + '</span>' + '<span>' + fileName + '</span>' // + '<span style="font-size: small;margin-left:15px;">'+size+'</span>'
                    //                             + '</a>' + '<span style="font-size: small;margin-left:15px;">' + size + '</span></div>';
                ...
                IM.HTML_pushMsg_addHTML(obj.msgType, you_sender, version,
                        ...
                        you_msgContent);
            } else if (obj.mcmEvent === 2) {// 2 end消息
                IM.HTML_pushMsg_addHTML(obj.msgType, you_sender, version,
                                IM._contact_type_m, b_current_contact_you,
                                name, '结束咨询');
            } else if (obj.mcmEvent === 3) {// 3发送mcm消息
                IM.HTML_pushMsg_addHTML(obj.msgType, you_sender, version,
                        IM._contact_type_m, b_current_contact_you, name,
                        you_msgContent);
            } else if (obj.mcmEvent === 3) {// 3发送mcm消息
                ...
                if (msgType === 1) {
                    ...
                    str = '你接收了一条位置消息...';
                } else if (msgType === 6 || msgType === 7) {// 统一按非压缩处理的
                    var url = obj.msgFileUrl;
                    var num = obj.msgFileSize;
                    var size = 0;
                    if (num < 1024) {
        ...
        DO_showFireMsg: function(id, msgtype) {
            ...
            num = window.setInterval(fireMsgTimer, 1000);
        ...
        DO_pop_photo_down: function() {
            ...
            var index = -1;
            msg.parent().find('div[msg="msg"][im_carousel="real"]:visible').each(function() {
                index++;
                if (IM._msgId === $(this).attr('id')) {
                    index++;
                    return false;
        ...
        HTML_sendMsg_addHTML: function(msg, msgtype, msgid, content_type,
            ...
            var display = msg === 'temp_msg' ? 'none' : 'block';
            var carou = '';
            if (msgtype === 4 || msgtype === 3) {
                carou = 'real';
            }
        ...
        HTML_resetHei: function() {
            ...
            // 绘制滤镜
            if ($('#pop_photo').css('display') === 'block') {
                IM.HTML_pop_photo_show();
            } else if ($('#pop').css('display') === 'block') {
                IM.HTML_pop_show();
            } else if ($('#lvjing').find('img').css('display') === 'block') {
                IM.HTML_LJ('black');
            } else if ($('#pop_takePicture').css('display') === 'block') {
                console.log('');
            } else {
                IM.HTML_LJ('black');
            }
        ...
        DO_im_attachment_file_up: function(id, oldMsgid) {
            ...
            if ($('#camera_button').find('i').hasClass('icon-picture')) {
                ...
                if (oFile.type.substring(0, oFile.type.indexOf('/')) === 'image') {
                    var windowWid = $(window).width();
                    var imgWid = 0;
                    var imgHei = 0;
                    if (windowWid < 666) {
        ...
        DO_chooseEmoji: function(unified, unicode) {
            ...
            $('#im_send_content').find('div').each(function() {
                if ($(this).html() === '<br>') {
                    $(this).replaceWith('<br>');
                } else {
        ...
        _snapshot: function() {
            ...
            IM.HTML_pop_takePicture_hide();
            if (resultObj.code === '174010') {// 没有调用applay方法
        ...
        DO_inviteCall: function(callType) {
            ...
            var view;
            if (callType === 1) {
                view = document.getElementById('receivedVideo');
                ...
                view = document.getElementById('voiceCallAudio');
        ...
        HTML_videoView: function(callId, caller, called, type) {
            ...
            if (windowWidth < 666) {
                ...
                if (type === 1) {
                    leftWidth = (windowWidth - $('#videoView').width()) / 2;
                    $('#videoView').css('left', leftWidth);
                    ...
                    leftWidth = (windowWidth - $('#voiceCallDiv_audio').width()) / 2;
